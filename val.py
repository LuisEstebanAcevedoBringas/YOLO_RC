...
def run(
    ...
    tp, fp, p, r, f1, ap, ap_class, ntn = ap_per_class(*stats, plot=plots, save_dir=save_dir, names=names)     
    ...
    LOGGER.info(pf % ('all', seen, ntn.sum(), mp, mr, map50, map))
    if nt.sum() == 0:
        LOGGER.warning(f'WARNING ⚠️ no labels found in {task} set, can not compute metrics without labels')
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            LOGGER.info(pf % (names[c], seen, ntn[c], p[i], r[i], ap50[i], ap[i]))
    # Print speeds
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=ROOT / '/home/bringascastle/Documentos/YMB/data/ucf.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'weights/ucf32fRC.pt', help='model path(s)')
    ...
    parser.add_argument('--save-json', action='store_true', help='save a COCO-JSON results file', default=False)
